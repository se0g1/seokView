...
kernel_slide_init_ios_heap_scan(kaddr_t kernel_region_base, kaddr_t kernel_region_end) {
	...
	for (;;) {
		...
		if (kr != KERN_SUCCESS) {
			printf("[+] FALSE kernel_slide_init_ios_heap_scan\n");
			return false;
		}
		...
		bool success = find_kernel_slide_from_heap_zone(address, size, kernel_region_base,
				kernel_region_end);
		if (success) {
			printf("[+] TRUE kernel_slide_init_ios_heap_scan\n");
			return true;
		}
next:
		address += size;
...
kernel_slide_init_ios_heap_scan_2_unsafe(kaddr_t kernel_region_base, kaddr_t kernel_region_end) {
	...
	if (!ok) {
		printf("[+]FALSE1 kernel_slide_init_ios_heap_scan_2_unsafe\n");
		return false;
	}
...
next:
	...
	if (kernel_ptr == (kaddr_t)(-1)) {
		printf("[+]FALSE2 kernel_slide_init_ios_heap_scan_2_unsafe\n");
		return false;
	}
	...
	for (kword_t slide = 0; slide < max_slide; slide += slide_increment) {
		...
		if (check_kernel_base(base)) {
			printf("[+] TRUE kernel_slide_init_ios_heap_scan_2_unsafe\n");
			return true;
		}
		printf("FALSE3 [+] kernel_slide_init_ios_heap_scan_2_unsafe\n");
	}
		return false;
}
...
kernel_slide_init_ios_unsafe_scan(kaddr_t region_base, kaddr_t region_end) {
	...
	if (PLATFORM_XNU_VERSION_GE(16, 3, 0)) {
		printf("[+]FALSE 1 kernel_slide_init_ios_unsafe_scan\n");
		return false;
	}
	...
	for (kword_t slide = min_slide; slide < max_slide; slide += slide_increment) {
		...
		if (check_kernel_base(base)) {
			printf("[+]TRUE 1 kernel_slide_init_ios_unsafe_scan\n");
			return true;
		}
	}
	printf("[+]FALSE 2 kernel_slide_init_ios_unsafe_scan\n");
	return false;
}
